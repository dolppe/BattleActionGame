...
bool UBattleGameplayAbility::CanActivateAbility(const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags,
	const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	// Don't set the actor info, CanActivate is called on the CDO
	UE_LOG(LogBattle, Log, TEXT("CanActivateAbility Start"));
	...
	if (!AbilitySystemGlobals.ShouldIgnoreCooldowns() && !CheckCooldown(Handle, ActorInfo, OptionalRelevantTags))
	{
		...
		UE_LOG(LogBattle, Log, TEXT("Ability could not be activated due to Cooldown: %s"), *GetName());
	...
	if (!AbilitySystemGlobals.ShouldIgnoreCosts() && !CheckCost(Handle, ActorInfo, OptionalRelevantTags))
	{
		...
		UE_LOG(LogBattle, Log, TEXT("Ability could not be activated due to Cost: %s"), *GetName());
	...
	{	// If the ability's tags are blocked, or if it has a "Blocking" tag or is missing a "Required" tag, then it can't activate.
		...
		UE_LOG(LogBattle, Log, TEXT("Ability could not be activated due to Blocking Tags or Missing Required Tags: %s"), *GetName());
	...
	if (!Spec)
	{
		UE_LOG(LogBattle, Warning, TEXT("CanActivateAbility %s failed, called with invalid Handle"), *GetName());
		return false;
	}
	...
	if (AbilitySystemComponent->IsAbilityInputBlocked(Spec->InputID))
	{
		...
		UE_LOG(LogBattle, Log, TEXT("Ability could not be activated due to blocked input ID %i: %s"), Spec->InputID, *GetName());
	...
	if (bHasBlueprintCanUse)
	{
		...
		{
			UE_LOG(LogBattle, Log, TEXT("CanActivateAbility %s failed, blueprint refused"), *GetName());
			return false;
		}
	}

	UE_LOG(LogBattle, Log, TEXT("CanActivateAbility End Is True"));
