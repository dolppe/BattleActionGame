...
void UBattleGamePhaseSubsystem::StartPhase(TSubclassOf<UBattleGamePhaseAbility> PhaseAbility,
	FBattleGamePhaseDelegate PhaseEndedCallback)
{
	UWorld* World = GetWorld();
	UBattleAbilitySystemComponent* GameStateASC = World->GetGameState()->FindComponentByClass<UBattleAbilitySystemComponent>();
	...
	if (GameStateASC)
	{
		...
		for (const FGameplayAbilitySpec* ActivePhase : ActivePhases)
		{
			...
			if (!InComingPhaseTag.MatchesTag(ActivePhaseTag))
			{
				...
				GameStateASC->CancelAbilityByFunc([HandleToEnd](const UBattleGameplayAbility* BattleAbility, FGameplayAbilitySpecHandle Handle)
				{
